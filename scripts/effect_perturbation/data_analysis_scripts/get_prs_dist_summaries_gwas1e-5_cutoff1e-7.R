...
#################
# Create output file 
sink('/deep_learning/aaw/051723/logs/get_prs_dist_summaries_gwas1e-5_cutoff1e-7.log')
...
#### Analysis ###
#################
prs_dist_metrics <- readr::read_csv("/deep_learning/aaw/051723/results/prs_dist_summaries/prs_dist_summaries_gwas1e-5_cutoff1e-07.csv")
skimr::skim(prs_dist_metrics[,-1])

old_prs_dist_metrics <- readr::read_csv("/deep_learning/aaw/042023/tables/vars_and_mags_df_gwas1e-5_cutoff1e-7.csv")
...
plot2 <- ggplot(new.vs.old.df,aes(x=Old, y=New)) +
  geom_point() +
  theme_bw() +
  geom_abline(slope=1,intercept=0,lty='dashed')+
  xlim(c(0,1)) + ylim(c(0,1)) +
  ggtitle('(GWAS,cutoff)=(1e-5,1e-7)')
